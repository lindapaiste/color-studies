import {GenericProps, I_Option} from "./types";
...
export type Props<T extends I_Option> = GenericProps<T[], any> & {
 ...
 */
export type UserProps<T extends I_Option> = GenericProps<T[], any> & Pick<Props<T>, 'showSelectAll'>

export const MultiSelect = <T extends I_Option>({value, onChange, showSelectAll = true, options, label, id = "multi-select", keyToObject, ...props}: Props<T>) => {
    const selectedKeys = (value || []).map(object => object.key);

    //can pass a keyToObject prop, but can also use find() on the array of options
    const getObject = ifDefined(keyToObject, (key: string) => options.find(o => o.key === key) as T);
        ...
        <div style={{display: "flex"}}>
            <FormControl>
                ...
                <Select
                    ...
                    value={selectedKeys}
                    variant="outlined"
                    onChange={(e) => onChange((e.target.value as string[] || []).map(getObject), e)}
                    renderValue={(keys) => (
                        <FlexRow wrap>
