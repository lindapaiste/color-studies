...
export const toKeyedWeights = (tuple: number[] | undefined): KeyedLCHWeights | undefined => {
    if (tuple === undefined) return undefined;
    const [lightness, chroma, hue] = tuple;
    return {lightness, chroma, hue};
}

export const makeEuclideanDifference = <CS extends ColorSpaceName>(colorSpace: CS): _DeltaEFormula<I_ColorAdapter, CS> =>
    (a, b, weights): number => {
...
export const deltaE00: _DeltaEFormula<I_ColorAdapter> = (a, b, weights): number => makeDeltaE00(weights)(a, b);

export const deltaE94: _DeltaEFormula<I_ColorAdapter> = (a, b, weights): number => makeDeltaE94(weights)(a, b);
