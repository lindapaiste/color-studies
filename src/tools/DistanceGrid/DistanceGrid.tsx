...
export const DistanceGrid = ({colors, calculator}: Props) => {
    const label = (hex: string) => (
        <TableCell key={hex} style={{backgroundColor: hex}}>{hex}</TableCell>
    );
 ...
 */
export const DistanceCell = ({a, b, calculator}: { a: I_ColorAdapter, b: I_ColorAdapter, calculator: I_DeltaECalculator }) => {
