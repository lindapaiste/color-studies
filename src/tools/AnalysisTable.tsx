...
import { analyzeGroupProperties } from "logic/classification/constraints/analyzeColors";
import { isNumberKey, round, typedKeys } from "lib";
import {
  GroupColorAnalysis,
  PropertyAnalysis,
} from "logic/classification/constraints/types";

// TODO: MUI table elements instead of DOM
  ...
  const columns = useMemo(
    () =>
      allGroupNames().map((group) => ({
        group,
        data: analyzeGroupProperties(getGroupHexes(group)),
      })),
    []
  );
  return <ResultsTable columns={columns} />;
};
...
export const ResultsTable = ({ columns }: Props) => {
  ...
  const isCompare = (group: string, property: string) =>
    compare.some((o) => o.group === group && o.property === property);
