...
export const calcSizes = (
  chartSize: Size,
  buckets: Bucket[]
): Pick<RenderProps, "colorWidth" | "colorHeight"> => {
  const barHeight = chartSize.height - 35; // fixed size label + margin
  const barWidth = 0.9 * (chartSize.width / buckets.length);
  const colorWidth = barWidth / COLUMNS_PER_BAR;
  // find the max height for each column, and use the minimum of the maxes
  const maxes = buckets.map(
    (bucket) => barHeight / Math.ceil(bucket.entries.length / COLUMNS_PER_BAR)
  );
 ...
 */
export const RenderAutoSizeHistogram = ({
  buckets,
  width = 500,
  height = 375,
  padPercent = 0,
}: Pick<RenderProps, "buckets"> & Partial<Size> & { padPercent?: number }) => {
