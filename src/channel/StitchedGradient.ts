...
import {I_ColorAdapter, PropColorArray} from "../color/types";
...
export type Props<CS extends ColorSpaceName> = Pick<GradientProps<CS>, 'model' | 'transform'> & PropColorArray;

export class StitchedGradient <CS extends ColorSpaceName> implements I_Gradient {
    ...
    constructor({colors, model, transform}: Props<CS>) {
        ...
        if ( colors.length === 0 ) {
            ...
            this.gradients.push( new ModelGradient({
                model,
                transform,
                start: colors[0],
                end: colors[0],
    ...
    colors(count: number): I_ColorAdapter[] {
        ...
        return flatMap( this.gradients, o => o.colors( countPer ) );
    }
}

export default StitchedGradient;
