...
export class GroupPerceptron
  implements I_GroupPerceptron, GroupColorClassifier<PerceptronResult>
{
  ...
  constructor({ group, channels, iterations = 15 }: PerceptronProps) {
    ...
    const trainingData = shuffledGroupData((hex) =>
      this.getFeatures(hexToColor(hex))
    );
    console.log(trainingData);
    // needs to iterate over training data multiple times - when to stop?
    for (let i = 0; i < iterations; i++) {
      trainingData.forEach((datum) =>
        this.model.train(datum.features, datum.group === group ? 1 : 0)
      );
    }
  ...
  private logPredict = (features: number[]): number => {
    ...
    for (let i = 2; i < this.model.weights.length; i++) {
      ...
      console.log(
        "adding " +
          this.model.weights[i] * features[i] +
          " for a new score of " +
          score
      );
    }
    ...
    console.log(
      "adding bias of " + this.model.bias + " for a final score of " + score
    );
    return score;
  };
