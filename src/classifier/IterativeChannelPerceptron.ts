...
import {AllChannelBoundaries, I_ChannelModel} from "./AllChannelBoundaries";
import {getBalancedSample} from "./shuffledData";
import {GroupedColor, I_Expected, PerceptronProps} from "./types";
...
import {TestedModel} from "./TestedModel";
...
export class IterativeChannelPerceptron {
    ...
    private readonly trainingData: (GroupedColor & I_Expected)[];

    private readonly iterations: number;
    private accuracy: I_ConfusionMatrix | undefined;
    public model: GroupPerceptron;

    constructor({group, iterations = 10, sampleSize = 100}: Omit<PerceptronProps, 'channels'>) {
        this.channels = [];
        this.group = group;
     ...
     */
    private maybeBetterModel = (channel: ChannelAdapter): TestedModel<GroupPerceptron> | false => {
        if (this.channels.includes(channel)) {
            return false;
        }
        //compare by balanced accuracy to see if it is an improvement
        const newer = this.modelWithChannel(channel);
        if (this.accuracy === undefined || newer.accuracy.balancedAccuracy > this.accuracy.balancedAccuracy) {
            return newer;
        } else {
            return false;
        }
     ...
     */
    private modelWithChannel = (channel: ChannelAdapter): TestedModel<GroupPerceptron> => {
        return new TestedModel(
            new GroupPerceptron({
                group: this.group,
...
export const makeIterativePerceptron = (group: string, sampleSize?: number, iterations?: number): GroupPerceptron => {
