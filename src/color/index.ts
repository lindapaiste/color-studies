 ...
 */
export const randomColors = (count: number): I_ColorAdapter[] => makeArray(count, randomColor);

/**
 * don't want to rely on the class itself, so avoid using new ColorAdapter in code
 */
export const hexToColor = (hex: string): I_ColorAdapter => new ColorAdapter(hex);

/**
 * convert a hex to color object while also accepting existing color objects
 */
export const eitherToColor = (
  color: string | I_ColorAdapter
): I_ColorAdapter => typeof color === "string" ? new ColorAdapter(color) : color;
