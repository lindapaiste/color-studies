 ...
 * Enjoy!
 */
export const rgbToRyb = (rgb: [number, number, number]): [number, number, number] => {
    ...
    // Normalize to values.
    let iMaxYellow = Math.max(iRed, iYellow, iBlue);
 ...
 * Enjoy!
 */
export const rybToRgb = (ryb: [number, number, number]): [number, number, number] => {
