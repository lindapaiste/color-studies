...
export const hueSet = (initial: Color, count: number, totalShift: number = 360): Color[] => {
    if ( count <= 1 ) return [initial]; //avoid divide by 0 error
    return [...new Array(count)].map(
        i => nextHue(initial, i * totalShift / (i - 1))
    );
};
 ...
 */
export const fixHue = (hue: number): number => {
    return hue > 0 ? hue % 360 : hue % 360 + 360;
};
